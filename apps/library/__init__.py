...
from apps.levels import MANAGER_LEVEL, VIEWER_LEVEL, current_level, tier
from apps.library.sync_from_sheet import DEFAULT_CSV_URL, sync_books_from_sheet

library = Blueprint("library", __name__, template_folder="templates")

# Books normally arrive via sync_from_sheet.sync_books_from_sheet() (an
# explicit, network-dependent admin action — DROP TABLE IF EXISTS books;
# CREATE TABLE books (...); re-import from the published Google Sheet), not
# via any schema block in this module. That's fine once a sync has run, but
# on a genuinely fresh library.db (first-run bootstrap, before anyone has
# synced yet) it meant *every* route touching `books` — /library, the
# dashboard, even the home page — raised "sqlite3.OperationalError: no such
# table: books" instead of just showing an empty catalog. Bootstrap an empty
# table here with the exact shape sync_from_sheet.py creates, so those
# routes work (empty state) before the first sync. CREATE TABLE IF NOT
# EXISTS is a safe no-op once real data exists — it never fights sync's own
# DROP+CREATE rebuild.
BOOKS_SCHEMA = """
...
def _ensure_books_table(conn, prefix=""):
    """Same {prefix} convention as _ensure_loans_table below — "" for this
    module's own get_conn() (library.db as main), "library." for
    dashboard.get_conn() (library.db attached under that alias)."""
    conn.executescript(BOOKS_SCHEMA.format(prefix=prefix))
    conn.commit()
